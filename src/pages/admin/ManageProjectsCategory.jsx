...
import { FolderTree, Plus, Edit2, Trash2, RefreshCw, Search, Hash, CheckCircle, XCircle } from 'lucide-react';
...
const ProjectCategory = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      Swal.fire({ title: 'Error', text: 'Failed to load project categories', icon: 'error', ...swalCfg(isDarkMode) });
  ...
  const handleDelete = async (item) => {
    const r = await Swal.fire({ 
      ...
      text: 'Projects in this category may become uncategorised.', 
      icon: 'warning', 
    ...
    <div className={`ssas-root ${isDarkMode ? 'dark' : 'light'}`}>
      ...
      <header className="ssas-header">
        <div className="ssas-header-left">
          <div className="ssas-header-icon" style={{ background: 'linear-gradient(135deg,#8b5cf6,#6d28d9)' }}><FolderTree size={20} /></div>
          <div>
            <h1 className="ssas-header-title">Project Categories</h1>
            <p className="ssas-header-sub">Organise your portfolio projects into categories</p>
          </div>
        </div>
        ...
        <div className="ssas-section-card">
          <div className="ssas-section-header">
            <div className="ssas-section-header-left">
              <div className="ssas-section-icon-wrap" style={{ background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.25)', color: 'var(--ss-purple)' }}><FolderTree size={18} /></div>
              <div>
                <h2 className="ssas-section-title">All Project Categories</h2>
          ...
          <div className="ssas-section-body" style={{ padding: 0 }}>
            {loading ? (
              ...
              <div className="ssas-empty-state"><FolderTree size={36} /><p>No project categories found</p><button className="ssas-btn-primary" onClick={openAdd}><Plus size={14} /> Add First Category</button></div>
            ) : (
                  ...
                  <tbody>
                    {filtered.map((cat, i) => (
                      <tr key={cat._id}>
                        ...
                        <td><span className="saap-name-cell"><FolderTree size={14} className="saap-row-icon saap-icon-purple" />{cat.name}</span></td>
        ...
        <div className="saap-modal-overlay" onClick={closeModal}>
          <div className="saap-modal" onClick={e => e.stopPropagation()}>
            <div className="saap-modal-header">
              <h3 className="saap-modal-title">{editing ? 'Edit Project Category' : 'New Project Category'}</h3>
            ...
            <div className="saap-modal-body">
              ...
              <div className="saap-form-group">
                ...
                <textarea className="ssas-input ssas-textarea" rows={3} value={form.description} onChange={e => handleFormChange('description', e.target.value)} placeholder="What kind of projects belong here?" />
              </div>
