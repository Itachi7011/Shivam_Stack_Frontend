...
const ProductCategory = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      Swal.fire({ title: 'Error', text: 'Failed to load product categories', icon: 'error', ...swalCfg(isDarkMode) });
    ...
    <div className={`ssas-root ${isDarkMode ? 'dark' : 'light'}`}>
      ...
      <header className="ssas-header">
        <div className="ssas-header-left">
          <div className="ssas-header-icon" style={{ background: 'linear-gradient(135deg,#10b981,#0891b2)' }}><Tag size={20} /></div>
          <div>
            <h1 className="ssas-header-title">Product Categories</h1>
            <p className="ssas-header-sub">Group your products into categories for better discovery</p>
          </div>
        </div>
      ...
      <div className="saap-page-body">
        <div className="saap-stats-row">
          <div className="saap-stat-card"><span className="saap-stat-val">{items.length}</span><span className="saap-stat-lbl">Total</span></div>
          <div className="saap-stat-card"><span className="saap-stat-val saap-green">{items.filter(i => i.isActive).length}</span><span className="saap-stat-lbl">Active</span></div>
          <div className="saap-stat-card"><span className="saap-stat-val saap-red">{items.filter(i => !i.isActive).length}</span><span className="saap-stat-lbl">Inactive</span></div>
        </div>

        <div className="ssas-section-card">
          <div className="ssas-section-header">
            <div className="ssas-section-header-left">
              <div className="ssas-section-icon-wrap" style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', color: 'var(--ss-green)' }}><Tag size={18} /></div>
              <div>
                <h2 className="ssas-section-title">All Product Categories</h2>
          ...
          <div className="ssas-section-body" style={{ padding: 0 }}>
            {loading ? (
              ...
              <div className="ssas-empty-state"><Tag size={36} /><p>No product categories found</p><button className="ssas-btn-primary" onClick={openAdd}><Plus size={14} /> Add First Category</button></div>
            ) : (
                  ...
                  <tbody>
                    {filtered.map((cat, i) => (
                      <tr key={cat._id}>
                        ...
                        <td><span className="saap-name-cell"><Tag size={14} className="saap-row-icon saap-icon-green" />{cat.name}</span></td>
                        ...
                        <td>
                          <button className={`saap-status-toggle ${cat.isActive ? 'saap-active' : 'saap-inactive'}`} onClick={() => handleToggle(cat)}>
                        ...
                        <td>
                          <div className="ssas-row-actions">
                            <button className="ssas-icon-btn ssas-btn-edit" onClick={() => openEdit(cat)}><Edit2 size={14} /></button>
                            <button className="ssas-icon-btn ssas-btn-del" onClick={() => handleDelete(cat)}><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        <div className="saap-modal-overlay" onClick={closeModal}>
          <div className="saap-modal" onClick={e => e.stopPropagation()}>
            <div className="saap-modal-header">
              <h3 className="saap-modal-title">{editing ? 'Edit Product Category' : 'New Product Category'}</h3>
            ...
            <div className="saap-modal-body">
              <div className="saap-form-group">
                ...
                <input className="ssas-input" value={form.name} onChange={e => handleFormChange('name', e.target.value)} placeholder="e.g. PDF Guides" />
              </div>
              <div className="saap-form-group">
                <label className="saap-label">Slug <span className="saap-hint">(auto-generated)</span></label>
                <input className="ssas-input saap-mono" value={form.slug} onChange={e => handleFormChange('slug', e.target.value)} placeholder="pdf-guides" />
              </div>
              <div className="saap-form-group">
                <label className="saap-label">Description</label>
                <textarea className="ssas-input ssas-textarea" rows={3} value={form.description} onChange={e => handleFormChange('description', e.target.value)} placeholder="What kind of products are in this category?" />
              </div>
